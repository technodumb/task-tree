...
int main() {
    ...
    { // undo/redo history over Forest snapshots (the "move a node back" feature)
        Forest f;
        TaskId a = f.addTask("a");
        History h;
        ...
        // Undoing a reparent moves the node back to its previous parent/slot.
        Forest m;
        TaskId r1 = m.addTask("r1");
        TaskId r2 = m.addTask("r2");
        TaskId kid = m.addTask("kid", r1);
        History hm;
        hm.snapshot(m);                       // checkpoint: kid under r1
        CHECK(m.reparent(kid, r2, 0), "reparent kid -> r2");
        CHECK(m.get(kid)->parent == r2, "kid now under r2");
        CHECK(hm.undo(m), "undo the move");
        CHECK(m.get(kid)->parent == r1, "undo put kid back under its previous parent");
