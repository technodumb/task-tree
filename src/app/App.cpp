...
void App::onKey(int key, int action, int mods) {
    ...
    if (mods & GLFW_MOD_CONTROL) {
        ...
        if (key == GLFW_KEY_Z && mode_ == Mode::Full && !searching_) {   // undo / redo
            if (mods & GLFW_MOD_SHIFT) redo(); else undo();
            return;
        }
        if (key == GLFW_KEY_Y && mode_ == Mode::Full && !searching_) { redo(); return; }
    }
...
void App::afterHistoryChange() {
    ...
    focusNode_ = 0;
    forceRelayout();
    if (searching_) updateSearchMatches();  // hit set may reference now-removed/re-added nodes
    save();
}
